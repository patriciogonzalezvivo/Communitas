...
public:
	...
	ofxParticle(int _idN, const ofxSpace *toSpace, ofVec2f _loc, bool _fromCenter = false){
		idN = _idN;
		lnk = NULL;
	...
	ofxParticle& setColor(ofVec2f _loc){ ofVec2f l = _loc - space->center; color.setHsb((ofRadToDeg(atan2(l.x,l.y)+PI)/360.0f)*255, 255, 255); return * this; };
